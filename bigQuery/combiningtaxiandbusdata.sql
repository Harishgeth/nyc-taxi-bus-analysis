...
create or replace table pytorch-checkout.ny_bus_data.aggregated_taxi_and_at_the_stop_bus_data as
select a.*,
b.LocationID,
...
join `pytorch-checkout.ny_bus_data.aggregated_taxi_data` c on a.timekey=c.timekey and b.LocationID=cast(c.pickup_location_id as Int64)
join `pytorch-checkout.ny_bus_data.aggregated_taxi_data` d on TIMESTAMP_SUB(a.timekey, INTERVAL 5 MINUTE) =d.timekey and b.LocationID=cast(d.pickup_location_id as Int64)
join `pytorch-checkout.ny_bus_data.aggregated_taxi_data` e on TIMESTAMP_SUB(a.timekey, INTERVAL 10 MINUTE) =e.timekey and b.LocationID=cast(e.pickup_location_id as Int64)
join `pytorch-checkout.ny_bus_data.aggregated_taxi_data` f on TIMESTAMP_SUB(a.timekey, INTERVAL 15 MINUTE) =f.timekey and b.LocationID=cast(f.pickup_location_id as Int64)
join `pytorch-checkout.ny_bus_data.aggregated_taxi_data` g on TIMESTAMP_SUB(a.timekey, INTERVAL 20 MINUTE) =g.timekey and b.LocationID=cast(g.pickup_location_id as Int64)
